...
namespace app_HackSpace.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public ActionResult YourHackathons()
        {
            var userName = User.Identity.Name;
            var hack = db.Hackathons.Where(h => h.UserName == userName);

            return View(hack);
        }

        public ActionResult SelectYourHackathons(int? id)
        {
            return RedirectToAction("SelectHack", "Home", new {id = id});
        }

        public ActionResult ParticipateHackathons()
        {
            var userName = User.Identity.Name;
            //var hack = db.Hackathons.Where(h => h.UserName == userName);

            var has_participant = db.has_participant.Include(hp => hp.Participant).Where(hp => hp.Participant.UserName == userName);
